...
<script>
	...
	function submitNovo(){
		window.location = 'doTemas.php?method=init';
	}
	
	function submitInsert() {
		document.cadastroForm.action="doTemas.php?method=inserir";
		document.cadastroForm.submit();
	}
	
	function submitAlterar() {
		document.cadastroForm.action="doTemas.php?method=alterar";
		document.cadastroForm.submit();
	}
	
	function submitEditarEvento(codEvento, codOcorrencia, codSub) {
		document.cadastroForm.action="doTemas.php?method=editar";
	...
	function submitExcluirEvento(codEvento, codOcorrencia, codSub) {
		if (confirm("Tem certeza que deseja excluir o tema " + codEvento + "?")) {
			document.cadastroForm.action="doTemas.php?method=excluir";
			document.cadastroForm.codEvento.value = codEvento;
			document.cadastroForm.codOcorrencia.value = codOcorrencia;
			document.cadastroForm.codSub.value = codSub;
			document.cadastroForm.submit();
...
<tr>
    <td><form name="cadastroForm" method="post" action="doTemas.php"> 
	...
	<table align="center" border="0" cellpadding="2" cellspacing="1" width="700">
      <tbody><tr bgcolor="#333333">
        <td colspan="2" height="25"><span style="color: rgb(255, 255, 255); font-weight: bold;">&nbsp;Cadastro Temas </span></td>
        </tr>
      <tr bgcolor="#f0f0f0">
	  ...
	  <tr>
		<td height="20" bgcolor="#f9f9f9"><b>Ensalamento: </b></td>
        <td valign="middle" bgcolor="#f9f9f9">
		Bloco <input name="numero_bloco" maxlength="40" size="10" value="<?php echo getAltPost(@$objSub->numero_bloco, 'numero_bloco', $try); ?>" type="text" >&nbsp;&nbsp;
		&nbsp;Andar <input name="andar" maxlength="8" size="10" value="<?php echo getAltPost(@$objSub->andar, 'andar', $try); ?>" type="text" >&nbsp;&nbsp;
		&nbsp;Sala <input name="numero_sala" maxlength="40" size="10" value="<?php echo getAltPost(@$objSub->numero_sala, 'numero_sala', $try); ?>" type="text" >
		</td>
      </tr>
      <tr>
      ...
      <tr align="right" bgcolor="#f0f0f0">
		...
		<input name="btNovo" id="btNovo" value="Novo" onclick="submitNovo();" type="button">
          &nbsp;&nbsp;&nbsp;	
				<input name="btInserir" id="btInserir" value="Inserir" onclick="submitInsert();" type="button" <?php if (getAltPost(@$objEvento->codigo, 'codEvento', $try) != "") echo "disabled"; ?>>
		&nbsp;&nbsp;&nbsp;	
		<input name="btAlterar" id="btAlterar" value="Alterar" onclick="submitAlterar()"  type="button" <?php if (getAltPost(@$objEvento->codigo, 'codEvento', $try) == "") echo "disabled"; ?>>
		</td>
      </tr>
    </table>
    </form></td>
	...
	<br>
		<table width="90%" align="center">
			...
			<tr >
				...
				<td width="5%" class="tableHeader">Bloco </td>
				<td width="5%" class="tableHeader">Andar </td>
				<td width="5%" class="tableHeader">Sala </td>
			...
			if ($numRows > 0) {
				...
				for ($i = 0; $i < $numRows; $i++) {
					...
					$bloco = mysql_result($resultado, $i, "numero_bloco");
					$andar = mysql_result($resultado, $i, "andar");
					$sala = mysql_result($resultado, $i, "numero_sala");
					...
					?>
						<tr bgcolor="<?php echo $bgcolor; ?>">
						  ...
						  <td align="center"><?php echo $bloco; ?></td>
						  <td align="center"><?php echo $andar; ?></td>
