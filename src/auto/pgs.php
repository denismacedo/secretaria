...
class pgs {
  ...
  function adicionar($item) {
	...
	foreach ($this->_itens as $i => $value) {
		if ($value['id'] == $item['id']) {
				return $this;
		}
	}
  ...
  function mostra ($args=array()) {
  	...
  	echo "<form name='_xclick' action='>";
echo "";
echo "";
echo "";
echo "<input type='hidden' name='item_name' value='Ursinho de pelúcia'>";
echo "<input type='hidden' name='amount' value='12.99'>";
    ...
    $_form = array();
    if ($args['open_form'])
      $_form[] = '<form action="https://www.paypal.com/br/cgi-bin/webscr" method="post">';
      $_form[] = '<input type="hidden" name="cmd" value="_xclick">';
      $_form[] = '<input type="hidden" name="business" value="sementeiradeluz@gmail.com">';

		$itemExtra = "";
		$total = 0;
      foreach ($this->_itens as $item) {
		$itemExtra .= " ".formatNumber($item['extra'], 5);
		$total +=$item['valor'];
   	  }

      
      $_form[] = '<input type="hidden" name="item_name" value="Inscrição CONCAFRAS:"'.$itemExtra.'>';
      $_form[] = '<input type="hidden" name="amout" value="'.$total.'">';

    if ($args['show_submit']) {
      if ($args['img_button']) {
        $_form[] = sprintf('  <input type="image" src="%s" name="submit" alt="Pague com o PayPal - &eacute; r&aacute;pido, gr&aacute;tis e seguro!"  />', $args['img_button']);
        ...
        $_form[] = '  <input type="submit" value="Efetuar Pagamento"  />';
		//$_form[] = sprintf ('  <input type="image" src="https://pagseguro.uol.com.br/Security/Imagens/%s"  name="submit" alt="Pague com o PagSeguro - &eacute; r&aacute;pido, gr&aacute;tis e seguro!" />', "btnPagarBR.jpg");
      }
    }
  ...
  function exibeCarrinho () {
    $i=1;
	$total = 0;
	echo "<br/><br/><table id='tbCarrinho' width='80%' align='center' ><thead><td></td><td>Inscrição</td><td>Detalhe</td><td>Valor</td></thead><tbody>";
    foreach ($this->_itens as $item) {
		$urlRemover = "payPalRemoverInscricao.php?id=".$item['id'];
		echo "<tr><td align='center'><a href='".$urlRemover."'>remover</a></td><td>".formatNumber($item['extra'], 5)."</td><td>",$item['descricao']."</td><td align='right'>".formatCurrency($item['valor'])."</td></tr>";
		$i++;
		$total +=$item['valor'];
    }
	echo "<tr><td colspan='3' align='right'><b>Total:</b></td><td align='right'><b>".formatCurrency($total)."</b></td></tr>";
	echo "</tbody></table>";
