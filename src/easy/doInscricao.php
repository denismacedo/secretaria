...
} else if ($metodo == "acompanhaInscricaoInit") {
	
	acompanhaInscricaoInit();
...
function init() {
	...
	// inicia com pais BR
	$paises = getPaises(true);
	$estados = getEstados(false, 'BR');
...
function inscricaoPasso2($codPF, $tipoMsg, $msg) {
	...
	if ($_SESSION["OCORRENCIA_SESSION"] != "" && $_SESSION["EVENTO_SESSION"] != "") {
		...
		$selectTipoAlojamento = "<select class='form-control form-control-line' name='tipoAlojamento' id='tipoAlojamento'><option value=''></option>";
		...
		if ($objEvento->tipoEvento == 11) {
			...
			$selectIdioma = "<select class='form-control form-control-line' name='idioma_tema_central' id='idioma_tema_central'></option>";
		...
		if ($objEvento->tipoEvento == '9') {
			$objInscricao->flag_trabalhador = 'S';
		}
		...
		$selectComissoes = "<select class='form-control form-control-line' name='comissoes' id='comissoes'><option value=''></option>";
		$linhas = mysql_num_rows($resultado);
		for($i = 0; $i < $linhas; $i++) {
			$strSel = "";
			...
			$selectComissoes .= "<option value='".mysql_result($resultado, $i, 0)."/".mysql_result($resultado, $i, 1)."' ".$strSel.$style.$disabled.">"
...
function salvaInscricao() {
	...
	$objInscricao->usuario_insercao = 'easy';
	
	
	if ($comissoes != "") {
		$objInscricao->flag_trabalhador = 'S';
	} else {
		$objInscricao->flag_trabalhador = 'N';
	}
	...
	//} else if ($objEvento->tipoEvento <> 9 && ((dateDiff($objPF->data_nasc, $objOcorrencia->inicio) >= 12 && ($temasAtuaisOp1 == "")) || $temasEspecificosOp1 == "")) {
	//	$erro = "INFORME OS TEMAS DE ESTUDO!";
	}
...
function getPaises($initBrasil) {

  $pais = getPost("pais");

  if ($pais == "" && $initBrasil == true) {
	  $pais = 'BR';
  }
...
function getEstados($echo, $pais) {
	...
	$retorno = '<select class="form-control form-control-line" name="uf" id="uf" onchange="changeEstado(document.cadastroForm.pais.value, this.value);" placeHolder="SELECIONE O ESTADO NA LISTA"><option></option>';

	$sql = "SELECT sigla, nome FROM estado where pais = '".$pais."' ORDER BY nome ";	
...
function getCidades($echo) {
  ...
  $retorno = '<select class="form-control form-control-line" name="cidade" id="cidade"><option></option>';
...
function salvar() {
	...
	$contato_responsavel = retiraCaracteresEspeciais(getPost("contato_responsavel"));
	...
	if ($nome == "" || $dataNasc == "" || $sexo == "" || $cidade == "" || $email == "") {
		$erro = "OS CAMPOS NOME, DATA DE NASCIMENTO, SEXO, CIDADE, EMAIL e PELO MENOS UM TELEFONE S&Atilde;O DE PREENCHIMENTO OBRIGAT&Oacute;RIO!";
	} else if (dateDiff(getDBDate($dataNasc), $objOco->inicio) < 12 && $contato_responsavel == "") {
        ...
        } else if (!validaNome($nome) || !validaNome($apelido)) {
		$erro = "INFORME O NOME SEM ACENTOS E SEM CARACTERES ESPECIAIS";
		...
		if ($objPF == "") {
		 ...
		 if ($erro == "") {
			...
			$objPF->contato_responsavel = $contato_responsavel;
...
function buscaInscricao() {
	...
	// BUSCA O OBJETO OCORRENCIA **************
	$objOcorrencia = $objOcorrencia->findByCodigo($evento, $ocorrencia);
...
function acompanhaInscricaoInit() {
	$objOcorrencia = new classOcorrencia();
